...
import { paint, mergeGeos, limb, mergeRanked, rankedInstances } from '../../core/geometry.js';
...
function pineconeGeo(N = 64, ws = 7, hs = 4) {
  ...
  for (let i = 0; i < N; i++) {
    ...
    const sw = 0.0115 * (0.55 + 0.45 * prof) * grow, sl = 0.0145 * (0.5 + 0.5 * prof) * grow;
...
/**
 * The full-detail spruce generator shared by the reference tree and the island's variants: trunk, root flares,
 * 17 whorls of branches with twigs, needle-spray cards and pinecones, around the base point (x, h0, z).
 * Returns { bark: [geometries; twig limbs tagged userData.twig], cards: [[matrix, colour]], cones: [matrices] }.
 * Draws from the shared random stream in a fixed order, so the reference tree is unchanged.
 * fallen: also scatter a few pinecones on the plot's ground around the trunk (the reference tree only).
 */
export function buildSpruce(x, z, h0, fallen) {
  const dens = isTouch ? 0.65 : 1;
  const bark = [], cards = [], branches = [];
  const top = h0 + 5.8;
  const twig = g => { g.userData.twig = true; return g; };
  bark.push(limb(new V(x, h0 - 0.1, z), new V(x, top, z), 0.2, 0.015, 12, 8));
  bark.push(limb(new V(x, h0 - 0.12, z), new V(x, h0 + 0.35, z), 0.34, 0.2, 12, 1));
  for (let i = 0; i < 5; i++) {
    const a = i / 5 * Math.PI * 2 + 0.4, d = new V(Math.cos(a), -0.35, Math.sin(a));
    bark.push(limb(new V(x, h0 + 0.12, z), new V(x + d.x * 0.55, h0 - 0.08, z + d.z * 0.55), 0.09, 0.02, 6));
  }
  ...
  function toneAt(p, tip, t) {
    const radial = Math.hypot(p.x - x, p.z - z) / (1.6 * (1 - t * 0.85) + 0.25);
  ...
  function branch(start, a, L, t, main) {
    ...
    for (let k = 0; k < nT; k++) {
      ...
      bark.push(twig(limb(bp, te, 0.0065, 0.0025, 3)));
  ...
  for (let i = 0; i < NW; i++) {
    ...
    for (let j = 0; j < nb; j++) branch(new V(x, y, z), phase + j * Math.PI * 2 / nb + rr(-0.3, 0.3), L0 * rr(0.75, 1.1), t, true);
    if (i < NW - 1) {
      ...
      for (let j = 0; j < ni; j++) branch(new V(x, y2, z), rng() * 6.28, L2 * rr(0.7, 1.1), t2, false);
    }
  }
  // leader shoot
  for (let k = 0; k < 26; k++) {
    const yy = top - 0.6 + rng() * 0.62;
    const p = new V(x + rr(-0.05, 0.05), yy, z + rr(-0.05, 0.05));
  ...
  for (let k = 0; k < 9; k++) {
    const a = rng() * 6.28, r = rr(0.45, 2.1), fx = x + Math.cos(a) * r, fz = z + Math.sin(a) * r;
    if (!fallen) break;
    if (Math.abs(fx) > 4.85 || Math.abs(fz) > 4.85) continue;
    const q = new THREE.Quaternion().setFromEuler(new THREE.Euler(Math.PI / 2 + rr(-0.2, 0.2), rng() * 6.28, 0, 'YXZ'));
    cones.push(new THREE.Matrix4().compose(new V(fx, H(fx, fz) + 0.02, fz), q, new V(1, 1, 1).multiplyScalar(rr(0.85, 1.1))));
  }
  return { bark, cards, cones };
}
...
/**
 * Full-detail spruce variants for the island: the reference generator with other seeds, built once in tree-local
 * space (base at the origin) and shared by every spruce that uses them. Each part carries a detail rank for
 * addThinning: outer / silhouette needle cards get low ranks (they survive longest), twig limbs and cones drop out
 * with distance, trunk and branches always stay. Materials are shared by all variants.
 */
export function createSpruceVariants(ctx, count, seed) {
  ...
  const barkDepth = depth(null, 0), nDepth = depth(needleTex, 0.42), coneDepth = depth(null, 0);
  addThinning(barkMat, false); addThinning(barkDepth, false); addThinning(nMat, true); addThinning(nDepth, true); addThinning(coneMat, true); addThinning(coneDepth, true);
  const plane = new THREE.PlaneGeometry(1, 1), coneBase = pineconeGeo(), coneLow = pineconeGeo(32, 5, 3);
  const out = [];
  for (let v = 0; v < count; v++) {
    setSeed(seed + v * 7919);
    const { bark, cards, cones } = buildSpruce(0, 0, 0, false);
    // outer cards (far from the trunk for their height) are the silhouette: low ranks
    const P = new V(), band = new Map(), bin = y => Math.floor(y / 0.3);
    cards.forEach(([m]) => { P.setFromMatrixPosition(m); const b = bin(P.y), r = Math.hypot(P.x, P.z); band.set(b, Math.max(band.get(b) || 0.01, r)); });
    const cardRank = cards.map(([m]) => { P.setFromMatrixPosition(m); const outer = clamp(Math.hypot(P.x, P.z) / band.get(bin(P.y))); return P.y > 5.3 ? 0 : rng() * (0.45 + 0.55 * (1 - outer)); });
    const barkSet = mergeRanked(bark, ['position', 'normal', 'uv'], g => (g.userData.twig ? 0.3 + 0.7 * rng() : 0));
    const needles = rankedInstances(plane, cards.map(c => c[0]), cardRank, cards.map(c => c[1]));
    const coneSet = rankedInstances(coneBase, cones, cones.map(() => 0)), coneLowSet = rankedInstances(coneLow, cones, cones.map(() => 0.92));
    let radius = 0, top = 0; cards.forEach(([m]) => { P.setFromMatrixPosition(m); radius = Math.max(radius, Math.hypot(P.x, P.z)); top = Math.max(top, P.y); });
    const bounds = new THREE.Sphere(new V(0, top / 2, 0), Math.hypot(radius, top / 2) + 0.4);
    [needles.geometry, coneSet.geometry, coneLowSet.geometry].forEach(g => { g.boundingSphere = bounds.clone(); });
    barkSet.geometry.computeBoundingSphere();
    out.push({
      name: 'spruce' + v, height: top + 0.3, width: 2 * radius + 0.4, trunkRadius: 0.34, bounds,
      cards: needles.count, cardRanks: needles.ranks,
      parts: [
        { geometry: barkSet.geometry, material: barkMat, depth: barkDepth, vertexRanks: barkSet.ranks },
        { geometry: needles.geometry, material: nMat, depth: nDepth, instances: needles },
        // pinecones: the reference cone (3.6k triangles) up close, the same shape with 32 scales (~0.7k) past 6 m
        { geometry: coneSet.geometry, material: coneMat, depth: coneDepth, instances: coneSet, maxDist: 6 },
        { geometry: coneLowSet.geometry, material: coneMat, depth: coneDepth, instances: coneLowSet, minDist: 6 },
      ],
    });
  }
  return out;
