...
import { WATER_Y, houseRectDist, inRocks, inRose, inSteps, CON, APP, H, H0, streamDist, footpathDist } from '../../world/layout.js';
...
export function createGrass(ctx) {
  const { scene } = ctx;
  {
    ...
    while (n < N && tries < N * 8) {
      ...
      scl[n * 3] = rr(0.03, 0.058); const fp = footpathDist(x, z); scl[n * 3 + 1] = streamDist(x, z) < 0.12 || fp < 0.01 ? 0 : tall * (0.3 + 0.7 * smooth(0.01, 0.45, fp)); scl[n * 3 + 2] = rr(0.3, 1.3);   // (none in the stream or on the path stones, shorter around them)
