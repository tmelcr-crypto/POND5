...
export function roseTextures() {
  ...
  return { petalTex, leafletTex, sepalTex };
}
/** The rose's part geometries (cupped / reflexed / flat petals, leaflet, sepal, thorn). No randomness. */
export function roseGeometries({ petalSegs = 8, leafSegs = 4, sepalSegs = 4, thornSides = 5 } = {}) {
  function petalGeo(cup, reflex, wave) { const g = new THREE.PlaneGeometry(1, 1, petalSegs, petalSegs); g.translate(0, 0.5, 0); const p = g.attributes.position; for (let i = 0; i < p.count; i++) { const x = p.getX(i), y = p.getY(i); p.setZ(i, cup * x * x * 4 * (0.35 + 0.65 * y) - reflex * y * y * y + wave * Math.sin(x * 16 + y * 3) * y * y * 0.05); } g.computeVertexNormals(); return g; }
  const petIn = petalGeo(0.26, -0.04, 0.12), petOut = petalGeo(0.13, 0.2, 0.3), petFlat = petalGeo(0.08, 0.02, 0.2);
  const lfG = new THREE.PlaneGeometry(1, 1, leafSegs >> 1, leafSegs); lfG.translate(0, 0.5, 0); { const p = lfG.attributes.position; for (let i = 0; i < p.count; i++) { const x = p.getX(i), y = p.getY(i); p.setZ(i, Math.abs(x) * 0.28 - y * y * 0.12); } lfG.computeVertexNormals(); }
  const spG = new THREE.PlaneGeometry(1, 1, 1, sepalSegs); spG.translate(0, 0.5, 0); { const p = spG.attributes.position; for (let i = 0; i < p.count; i++) { const y = p.getY(i); p.setZ(i, -0.25 * y * y); } spG.computeVertexNormals(); }
  const thornG = new THREE.ConeGeometry(0.0028, 0.011, thornSides); thornG.translate(0, 0.0055, 0);
  return { petIn, petOut, petFlat, lfG, spG, thornG };
}
/**
 * The rose generator shared by the reference bush and the island's wild roses: arching canes with thorns, compound
 * leaves, spiral-petal roses and buds around `base`. ground(x, z): also drop fallen petals on that ground (the
 * reference bush; pass null to skip); NC canes. Returns the part lists; draws from the shared random stream in a fixed order.
 */
export function buildRose(base, ground, NC = 24) {
  const canes = [], thorns = [], leaflets = [], pin = [], pout = [], sep = [], stam = [], hips = [], fallen = [];
  ...
  if (ground) for (let i = 0; i < 22; i++) { const a = rng() * 6.28, r = rr(0.12, 0.6), x = base.x + Math.cos(a) * r, z = base.z + Math.sin(a) * r; const q = new THREE.Quaternion().setFromEuler(new THREE.Euler(-Math.PI / 2 + rr(-0.25, 0.25), 0, rng() * 6.28, 'XYZ')); const s = rr(0.028, 0.04); fallen.push([new THREE.Matrix4().compose(new V(x, ground(x, z) + 0.012, z), q, new V(s * 0.95, s, s)), new THREE.Color(rr(0.6, 0.85), rr(0.6, 0.8), rr(0.6, 0.8))]); }
  return { canes, thorns, leaflets, pin, pout, sep, stam, hips, ground: fallen };
}
...
/**
 * Wild roses for the island: the reference generator (buildRose) and materials, one bush per variant in local space
 * (base at the origin), as parts for plantGroups() like the trees. Lighter than the plot's bush: 14 canes instead of 24,
 * coarser petal / leaflet / sepal cards, and the solid parts (canes, thorns, stamens, hips) and the petals each baked
 * into one vertex-coloured mesh, so a rose is four draw calls. Detail ranks drop thorns, stamens and some leaflets
 * first (addThinning with `thin`, THIN_SMALL), then the bush dissolves into its billboard.
 */
export function createRoseVariants(ctx, count, seed, thin) {
  setSeed(seed);
  const { petalTex, leafletTex, sepalTex } = roseTextures();
  ...
  const cane = lin(0x55682c), thorn = lin(0x8a3b26), stamen = lin(0xf0c030), hip = lin(0x4a6a28), white = new THREE.Color(1, 1, 1);
  // one baked copy of geo per instance, with its colour (and uv for textured parts)
  const bake = (geo, list, base, attrs) => list.map(e => {
    const [m, c] = Array.isArray(e) ? e : [e, white], g = geo.clone().applyMatrix4(m), col = base.clone().multiply(c), n = g.attributes.position.count, C = new Float32Array(n * 3);
    for (let k = 0; k < n; k++) col.toArray(C, k * 3);
    g.setAttribute('color', new THREE.BufferAttribute(C, 3)); return attrs.includes('uv') ? g : g.deleteAttribute('uv');
  });
  const out = [];
  for (let v = 0; v < count; v++) {
    setSeed(seed + 17 + v * 6151);
    const b = buildRose(new V(0, -0.02, 0), null, 14);
    const solidAttrs = ['position', 'normal', 'color'], petalAttrs = ['position', 'normal', 'uv', 'color'];
    const canes = b.canes.map(g => { const n = g.attributes.position.count, C = new Float32Array(n * 3); for (let k = 0; k < n; k++) cane.toArray(C, k * 3); g.setAttribute('color', new THREE.BufferAttribute(C, 3)); return g; });
    const solidList = canes.map(g => [g, 0])
      .concat(bake(thornG, b.thorns, thorn, solidAttrs).map(g => [g, rr(0.55, 1)]))
      .concat(bake(stamG, b.stam.map(([m, c]) => [m.multiply(S), c]), stamen, solidAttrs).map(g => [g, rr(0.5, 1)]))
      .concat(bake(hipG, b.hips, hip, solidAttrs).map(g => [g, 0]));
    const solids = mergeRanked(solidList.map(x => x[0]), solidAttrs, (g, i) => solidList[i][1]);
    const petals = mergeRanked(bake(petIn, b.pin, white, petalAttrs).concat(bake(petOut, b.pout, white, petalAttrs)), petalAttrs, () => 0);
    const set = (geo, list, rank) => rankedInstances(geo, list.map(e => e[0]), list.map(rank), list.map(e => e[1]));
    const lf = set(lfG, b.leaflets, () => rng() * 0.9), sp = set(spG, b.sep, () => rr(0, 0.6));
    // bounds from the leaflets and flowers (the canes stay inside them)
    const P = new V(); let radius = 0, top = 0;
    b.leaflets.concat(b.pout).forEach(([m]) => { P.setFromMatrixPosition(m); radius = Math.max(radius, Math.hypot(P.x, P.z)); top = Math.max(top, P.y); });
    const bounds = new THREE.Sphere(new V(0, top / 2, 0), Math.hypot(radius, top / 2) + 0.12);
    const parts = [
      { geometry: solids.geometry, material: solidMat, depth: solidDepth, vertexRanks: solids.ranks, castShadow: false },   // canes: too thin to show in the shadow map
      { geometry: lf.geometry, material: lMat, depth: lDepth, instances: lf },
      { geometry: petals.geometry, material: pMat, depth: pDepth, vertexRanks: petals.ranks },
      { geometry: sp.geometry, material: sMat, depth: sDepth, instances: sp, castShadow: false },
    ];
    parts.forEach(p => { p.geometry.boundingSphere = bounds.clone(); });
    out.push({ name: 'rose' + v, height: top + 0.1, width: 2 * radius + 0.2, radius, bounds, cards: lf.count, cardRanks: lf.ranks, parts });
  }
  return out;
}
