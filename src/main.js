...
createMeadowFlowers(ctx);
const flies = createButterflies(ctx);
createRockOutcrop(ctx);
createRoseBush(ctx);
...
function frame(now) {
  ...
  for (let i = 0; i < pads.length; i++) { const p = pads[i]; p.position.y = WATER_Y + 0.006 + Math.sin(t * 1.3 + p.userData.ph) * 0.0035; p.rotation.y += Math.sin(t * 0.4 + p.userData.ph) * 0.0006; }
