 ...
 * the same room, say), set(k, e) }, where set shows it burning k 0..1 with embers e 0..1 (hot) and is called whenever
 * those change.
 */
...
export function createFires({ st }) {
  ...
  let shown = null, aimed = null, check = 0;
  function showBtn(f) {
    const state = f ? (f.lit ? 'out' : 'light') : null; if (state === shown) return; shown = state;
    btn.style.display = state ? '' : 'none'; if (!state) return;
    btn.innerHTML = state === 'out' ? ICON_OUT : ICON_LIGHT; btn.setAttribute('aria-label', state === 'out' ? 'Put the fire out' : 'Light the fire');
  }
  function toggle(f = aimed) { if (!f) return; f.lit = !f.lit; save(); showBtn(f); }
  ...
  function inView() {
    if (!st.walk || !st.playing || st.aboard || st.inBed || st.seat || st.chestOpen) return null;
  ...
  return {
    fires,
    add(f) {
      const lit = saved[f.id] !== undefined ? !!saved[f.id] : true;
