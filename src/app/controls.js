...
import { WATER_Y, BOTTOM, HOUSE, PAD_H, CB, roofY, rockColliders, CON, APP, H } from '../world/layout.js';

/**
 * First-person free-flight controls and the settings panel:
 *  - desktop: pointer lock / drag look, WASD fly, Space/Q up/down, Shift boost, wheel speed, F door, L lights
 *  - touch: left-half virtual joystick, right-half look, Up/Down buttons
 *  - collisions with terrain, the slab, tree trunks, rock outcrop and the cabin (walls, roof, door, floor)
 */
export function createControls(app) {
  const { canvas, camera, cabin, toggleDoor, setLights, setSun, scheduleEnv } = app;
  const st = { yaw: 0, pitch: -0.14, pos: new V(4.3, 1.45, 4.5), vel: new V(), keys: {}, speed: 2.2, locked: false, drag: false, joy: { x: 0, y: 0 }, up: 0, down: 0, playing: false };
  ...
  function inside(x, z) { return Math.abs(x) < 5.04 && Math.abs(z) < 5.04; }
  function houseCollide(np, prev) {
    ...
    if (Math.abs(ix) < 1.58 && Math.abs(iz) < 1.23) { const fy = PAD_H + CB.FL + 0.28; if (np.y < fy && np.y > PAD_H - 0.4) { np.y = fy; st.vel.y = Math.max(0, st.vel.y); } }
  }
  function move(dt) {
    const K = st.keys;
    ...
    wish.set(0, 0, 0).addScaledVector(fwd, mf).addScaledVector(right, mr).addScaledVector(UPV, mu);
    if (wish.lengthSq() > 1) wish.normalize();
    const sp = st.speed * (K.ShiftLeft || K.ShiftRight ? 2.6 : 1);
    st.vel.lerp(wish.multiplyScalar(sp), 1 - Math.exp(-dt * 7));
    np.copy(st.pos).addScaledVector(st.vel, dt);
    if (inside(np.x, np.z)) {
      const top = Math.max(H(np.x, np.z), WATER_Y) + 0.26, bot = BOTTOM - 0.22;
      if (np.y < top && np.y > bot) {
        if (inside(st.pos.x, st.pos.z)) { if (st.pos.y >= (top + bot) / 2) { np.y = top; st.vel.y = Math.max(st.vel.y, 0); } else { np.y = bot; st.vel.y = Math.min(st.vel.y, 0); } }
        else { np.x = st.pos.x; np.z = st.pos.z; st.vel.x = st.vel.z = 0; }
      }
    }
    ...
    np.x = clamp(np.x, -16, 16); np.z = clamp(np.z, -16, 16); np.y = clamp(np.y, -9, 14);
    st.pos.copy(np);
    camera.position.copy(st.pos);
    camera.rotation.set(st.pitch, st.yaw, 0);
  }
  return { st, move, fmtTime, setSpeed, timeIn, timeV };
}
